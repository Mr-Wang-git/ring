...
<html lang="en">
  ...
  <body id="reportsPage">
              ...
              <table class="table table-hover tm-table-small tm-product-table">
                ...
                <c:forEach items="${user}" var="user">
	                  ...
	                  <!-- 装饰 -->
	                    ...
	                    <td>
	                      <!-- 删除此行 -->
	                      <a href="javascript:void(0);" onclick="del(${user.id},${user.resumeid},${user.userinfoid})" class="tm-product-delete-link">
	                        <i class="fa fa-trash-alt tm-product-delete-icon"></i>
	                      </a>
	                      
	                      <!-- 修改此行 -->
	                      <a href="javascript:void(0);" onclick="update(${user.id})" class="tm-product-delete-link">
    ...
    <script type="text/javascript">
	                    ...
	                    var rid;
	                    var infoid;
	              		function del(id1,resumeid,infoid1){
	              			/* 显示 */
	              		...
	              		function method1(){
	              			/* 删除操作 */
                  			$.post("deleteThis",
                  			{"id":id,"rid":rid,"infoid":infoid},
                  			function(data){
                  				if(data==true){
                  					alert("已删除ID为"+id+"的用户")
                  					location.href="userTable"
                  				}
                  			},"json")
                  		}
	...
	<div id="add" style="border: 1px solid;width: 100%;margin: auto;height: 100%;position: fixed;left: 0px;top: 0px;background: rgb(0,0,0,0.6);overflow: auto;text-align: center;display: none;">
		<div style="background: white;width: 400px;height: 400px;margin:0 auto;margin-top: 10%;border-radius: 5px;">
			<div style="height: 360px;border-bottom: 1px solid #CCCCCC;">
				<!-- 框内内容 -->
				<div style="font-size: 0.9rem;padding-top: 30px;">您要添加用戶嗎？</div>
				<br/>
					<h5>账号:</h5><input type="text" class="form-control" id="acc" name="acc" placeholder="Account(个人邮箱更容易记忆)" >
					<h5>密码:</h5><input type="text" class="form-control" id="password" name="pass"  placeholder="password(新密碼)">
					<h5>手机号:</h5><input type="text" class="form-control" id="phone" name="phone" placeholder="Cell-phone Number(手机号，用于找回密码使用)">
				<br/>
			</div>
			<div style="height: 39px;">
	...
	function method3(){
			...
			if(!isStr){
				...
				/* 添加操作 */
				$.post("insertUser",
              			{"account":account,"pass":password,"phone":phone},
              			function(data){
              				if(data==true){
              					alert("已添加账号为"+account+"的用户")
              					location.href="userTable"
              				}
              			},"json") 
